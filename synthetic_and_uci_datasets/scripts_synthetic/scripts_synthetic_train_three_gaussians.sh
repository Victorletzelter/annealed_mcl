    ...
    for hyp in "${hyps[@]}"; do
        ...
        if [ "$train_exp" = "True" ]; then
            for temperature_ini in "${temperatures_ini[@]}" ; do
                for temperature_decay in "${temperatures_decay[@]}" ; do
                    ...
                    python train.py data=synthetic_data.yaml model=WTABased.yaml experiment=A_${experiment}.yaml hydra.job.name=data_${experiment}_model_Score_based_WTA_${num_hypotheses}_hyps_annealed_exp_${temperature_ini}_${temperature_decay}_decay model.num_hypothesis=${num_hypotheses} trainer.max_epochs=${max_epochs} logger.mlflow.experiment_name=stable_Annealing_${experiment}_train_${max_epochs}_epochs model.hparams.training_wta_mode=stable_awta model.hparams.temperature_ini=${temperature_ini} model.hparams.temperature_decay=${temperature_decay} model.hparams.scheduler_mode=exponential model.hparams.plot_mode=True model.restrict_to_square=True model.hparams.learning_rate=0.01 model.hparams.optimizer=${optimizer}
                done
            done
        fi
        ...
        if [ "$train_linear" = "True" ]; then
            for temperature_ini in "${temperatures_ini[@]}" ; do
                ...
                python train.py data=synthetic_data.yaml model=WTABased.yaml experiment=A_${experiment}.yaml hydra.job.name=data_${experiment}_model_Score_based_WTA_${num_hypotheses}_hyps_annealed_linear_${temperature_ini}_ini model.num_hypothesis=${num_hypotheses} trainer.max_epochs=${max_epochs} logger.mlflow.experiment_name=stable_Annealing_${experiment}_train_${max_epochs}_epochs model.hparams.training_wta_mode=stable_awta model.hparams.temperature_ini=${temperature_ini} model.hparams.scheduler_mode=linear model.hparams.plot_mode=True model.restrict_to_square=True model.hparams.learning_rate=0.01 model.hparams.optimizer=${optimizer}
            done
        fi      

        if [ "$train_other" = "True" ]; then
            # Score_based_WTA model without annealing
            python train.py data=synthetic_data.yaml model=WTABased.yaml experiment=A_${experiment}.yaml hydra.job.name=data_${experiment}_model_Score_based_WTA_${num_hypotheses}_hyps model.num_hypothesis=${num_hypotheses} trainer.max_epochs=${max_epochs} logger.mlflow.experiment_name=stable_Annealing_${experiment}_train_${max_epochs}_epochs model.hparams.plot_mode=True model.hparams.training_wta_mode=wta model.restrict_to_square=True model.hparams.learning_rate=0.01 model.hparams.optimizer=${optimizer}
        fi

        if [ "$train_epsilon" = "True" ]; then
            # epsilon-WTA model
            python train.py data=synthetic_data.yaml model=WTABased.yaml experiment=A_${experiment}.yaml hydra.job.name=data_${experiment}_model_Score_based_WTA_${num_hypotheses}_hyps_epsilon_${training_epsilon} model.num_hypothesis=${num_hypotheses} trainer.max_epochs=${max_epochs} logger.mlflow.experiment_name=stable_Annealing_${experiment}_train_${max_epochs}_epochs model.hparams.training_wta_mode=wta-relaxed model.hparams.training_epsilon=${training_epsilon} model.hparams.scheduler_mode=linear model.hparams.plot_mode=True model.restrict_to_square=True model.hparams.optimizer=${optimizer}
        fi
    done
done