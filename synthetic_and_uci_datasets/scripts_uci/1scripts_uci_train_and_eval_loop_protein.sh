...
declare -a temperature_lim=(0.0005)
        ...
        do
            ...
            do
                if [[ "$scheduler_mode" == "linear" ]]
                    then
                        python train.py data=uci_data.yaml experiment=uci/uci_${dataset_name}.yaml data.hparams.data_root=${MY_HOME}/data/uci model=WTABased.yaml model.num_hypothesis=${hyp_count} model.square_size=10 hydra.job.name=annealed_wta-${hyp_count}-hyps-seed-1-split-${split_num}_${max_epochs}e_lr${lr}_0.8train_temperature_${temperature_ini}_scheduler_${scheduler_mode}_optimizer_${optimizer}_wta_valid_temp_lim_${temperature_lim}_wta_lim seed=1 data.hparams.split_num=${split_num} logger.mlflow.experiment_name=A_0.8train_FULL_uci_${dataset_name}_hopt_scaled_${max_epochs}e_lr${lr}_num_hyps_${hyp_count} trainer.max_epochs=${max_epochs} model.hparams.learning_rate=${lr} model.hparams.compute_nll=False model.hparams.h_optimization=False model.hparams.training_wta_mode="stable_awta" model.hparams.temperature_ini=${temperature_ini} model.hparams.scheduler_mode=${scheduler_mode} early_stopping.patience=1000 trainer.check_val_every_n_epoch=10 model.hparams.optimizer=${optimizer} model.hparams.wta_validation=True model.hparams.temperature_lim=${temperature_lim} model.hparams.wta_after_temperature_lim=True
                    fi

                    if [[ "$scheduler_mode" == "exponential" ]]
                    then
                        python train.py data=uci_data.yaml experiment=uci/uci_${dataset_name}.yaml data.hparams.data_root=${MY_HOME}/data/uci model=WTABased.yaml model.num_hypothesis=${hyp_count} model.square_size=10 hydra.job.name=annealed_wta-${hyp_count}-hyps-seed-1-split-${split_num}_${max_epochs}e_lr${lr}_0.8train_temperature_${temperature_ini}_scheduler_${scheduler_mode}_optimizer_${optimizer}_decay_${temperature_decay}_wta_valid_temp_lim_${temperature_lim}_wta_lim seed=1 data.hparams.split_num=${split_num} logger.mlflow.experiment_name=A_0.8train_FULL_uci_${dataset_name}_hopt_scaled_${max_epochs}e_lr${lr}_num_hyps_${hyp_count} trainer.max_epochs=${max_epochs} model.hparams.learning_rate=${lr} model.hparams.compute_nll=False model.hparams.h_optimization=False model.hparams.training_wta_mode="stable_awta" model.hparams.temperature_ini=${temperature_ini} model.hparams.scheduler_mode=${scheduler_mode} model.hparams.temperature_decay=${temperature_decay} early_stopping.patience=1000 trainer.check_val_every_n_epoch=10 model.hparams.optimizer=${optimizer} model.hparams.wta_validation=True model.hparams.temperature_lim=${temperature_lim} model.hparams.wta_after_temperature_lim=True
                    fi
            done

            for epsilon in "${epsilons[@]}"
            do
                python train.py data=uci_data.yaml experiment=uci/uci_${dataset_name}.yaml data.hparams.data_root=${MY_HOME}/data/uci model=WTABased.yaml model.num_hypothesis=${hyp_count} model.square_size=10 hydra.job.name=epsilon_wta-${hyp_count}-hyps-seed-1-split-${split_num}_${max_epochs}e_lr${lr}_0.8train_epsilon_${epsilon} seed=1 data.hparams.split_num=${split_num} logger.mlflow.experiment_name=A_0.8train_FULL_uci_${dataset_name}_hopt_scaled_${max_epochs}e_lr${lr} trainer.max_epochs=${max_epochs} model.hparams.learning_rate=${lr} model.hparams.h_optimization=False model.hparams.training_wta_mode="wta-relaxed" model.hparams.training_epsilon=${epsilon} early_stopping.patience=1000 trainer.check_val_every_n_epoch=10
            done
        done
    done
done